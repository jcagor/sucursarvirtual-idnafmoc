import { ProgramObjectList } from "domain/models/ProgramType";
...
export default class findAllProgramsUseCase {
  ...
  async execute(accessToken?: string): Promise<ProgramObjectList | undefined> {
    if (!accessToken) return;
    const request = await this.rannRepository
      .findAllPrograms(accessToken)
      .catch((error) => error);
