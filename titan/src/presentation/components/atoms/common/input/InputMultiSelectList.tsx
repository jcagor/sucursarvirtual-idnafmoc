...
import { KnowledgeAndSkills, multiSelectValues } from "domain/models";
...
interface Props {
  ...
  codeLabel?: boolean;
  clear?: boolean;
  errors?: string | string[] | FormikErrors<KnowledgeAndSkills>[] | FormikErrors<SelectOption>[]| undefined;
}

export const InputMultiSelectList: React.FC<Props> = ({
  name,
  ...
  const parseFormError = (error: string | string[] | FormikErrors<KnowledgeAndSkills>[] | undefined) =>{
    if (!error){
      return "";
    }
  ...
  const filter = () => {
    ...
    data.forEach((item: {label: string; value: string; shorthand?: string }) => {      
        options.push({
          value: item.value,
          label: item.label,
  ...
  const customStyles = {
    placeholder: (prev:CSSObjectWithLabel) => ({
      ...prev,
      color: '#CCCCCC', // your desired color      
    }),
  ...
  return (
    <div className="w-full h-full">
      ...
      <Select
        ...
        className="placeholder-principal-440"
      />
...
export default InputMultiSelectList;
