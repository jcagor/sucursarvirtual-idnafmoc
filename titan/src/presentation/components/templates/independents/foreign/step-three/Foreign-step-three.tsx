...
export const ForeignStepThree = () => {
  ...
  const getOptions = async () => {
    if (
      ...
      !independentForm.independentForm.TituloNacionalidad ||
      !independentForm.independentForm.ForeignNationality ||
    ...
    if (isEmptyArray(errorsFirtForm) && isEmptyArray(errorsFirtForm1)) {
      independentAux = {
        ...
        Direccion: formikForm.values.compuesta
          ...
          .replaceAll("-", "")
          .replaceAll("  ", " "),
        DireccionRestante: `${formikForm.values.direccion} ${formikForm.values.detalle}`,
      };
  ...
  return (
    ...
    >
      <StepSection
        number={4}
        descriptionStep="Datos de Ubicación en Colombia"
        className="w-4/5"
      />
