...
export const EntityStepFour = () => {
  ...
  const formModuleOnePacsSchema = Yup.object().shape({
    ...
    modality: Yup.string().required("Campo obligatorio *"),
  });
  ...
  const getOptions = async () => {
    ...
    const occupations = await ocupationUseCase.execute(session?.access_token);
  ...
  const sendData = async () => {
    ...
    let indepFormAux = {
      ...independentForm.independentForm,
      CategoriaAportante: formikForm.values.modality == "0" ? "I2" : "I1",
    ...
    const entity = {
      "Tipo de identificación": indepFormAux.TipoIdentEntidad,
      "Número de identificación": indepFormAux.NIFEntidad,
      "Nombre de la entidad": indepFormAux.TituloEntidad,
    };
        ...
        proof: [
          {
            "Tipo de independiente": {
              "Tipo de independiente": "Pertenece a una entidad",
            },
          },
