...
import {
  AFFILIATIONS_INDEPENDENT_MESSAGE_TYPE,
...
const GoIndependent = () => {
  ...
  // ---- states
  const [isLoadingIndependent, setIsLoadingIndependent] = useState(false);
  const [messageIndependent, setMessageIndependent] = useState("");
  ...
  const handlerIndependent = async () => {
    if (isLoadingIndependent) return;
    setIsLoadingIndependent(true);
    try {
      const requestsOnProcess = await findAllRequestsCase.execute(
        ...
        undefined,
        {
          ...
          status: FILED_STATUS_IN_PROGRESS_CODE.toString(),
        }
      );
      const requestsOnRadicate = await findAllRequestsCase.execute(
        ...
        undefined,
        {
          documentType: decodedToken?.identification_type,
          document: decodedToken?.identification_number,
          status: FILED_STATUS_FILED_CODE.toString(),
        }
      );
      ...
      if (
        ...
        return router.push("independents/step-one");
      } else if (
        verifyRightsResponse?.TitularPrimerNombre != null &&
        (verifyRightsResponse.TitularEstado?.includes("VIGEN") ||
          verifyRightsResponse.TitularEstado?.includes("SUSPE")) &&
        ...
        const message =
          AFFILIATIONS_INDEPENDENT_MESSAGE_TYPE[
            verifyRightsResponse.TitularGrupoAfiliado! as keyof typeof AFFILIATIONS_INDEPENDENT_MESSAGE_TYPE
          ];
        setMessageIndependent(message);
        handleOpenModal(true);
        return;
      }

      router.push("independents/step-one");
    } catch (error) {
      console.log(error);
      setMessageIndependent(
  ...
  return (
    <React.Fragment>
      <ActionMenuCard
        height={80}
        width={80}
        action={handlerIndependent}
        urlImage={"/icons/heand_with_folder_icon.svg"}
        name={"Afíliate como independiente"}
        loading={isLoadingIndependent}
      />
