...
const optionsModality = [
  { label: "Afiliados aportantes 2%", value: "P1" },
  { label: "Afiliados aportantes 0.6%", value: "P4" },
];
const optionsOther = [
  { label: "SI", value: "0" },
  { label: "NO", value: "1" },
...
const fieldsRequired = [
  ...
  {
    name: "modality_affiliation",
    label: "La modalidad de aporte es requerida",
  },
];
...
const LoadComplementFiles = () => {
  ...
  const [selectedValues, setSelectedValues] = useState<{
    ...
    modality_affiliation: SelectOption;
  }>({
    salary: "",
    code_eps: null,
    other_affiliation: null,
    modality_affiliation: null,
  });
  const [files, setFiles] = useState<{
    ...
    document_pension_discount: File;
  }>({
    ...
    document_pension_discount: new File([], "empty.PDF", { type: ".PDF" }),
  });
  const [errors, setErrors] = useState<{
    ...
    document_pension_discount: string;
  }>({
    salary: "",
    code_eps: "",
    other_affiliation: "",
  ...
  const { saveFinalInformation, saveRequestResume } =
    ...
    try {
      ...
      if (Object.values(errorLoad).every((error) => error === "")) {
        const income = {
          ...
          code_eps: selectedValues.code_eps,
          other_affiliation: selectedValues.other_affiliation,
        };
        saveFinalInformation(
          files,
          income,
          selectedValues.modality_affiliation
        );
        ...
        const filesToSend: File[] = [
          ...
          files.document_pension_discount,
        ];
        const proof = [
          {
            Entidad: {
              "Tipo de documento": "NIT",
              "Número de documento": data.entity.Nit,
              "Nombre de la entidad": data.entity.RazonSocial,
            },
          },
          {
            "Información de tus ingresos": {
              "Ingreso económico": selectedValues.salary,
              "Modalidad de aporte": selectedValues.modality_affiliation?.label,
              EPS: selectedValues.code_eps?.label,
              "Has estado afiliado a otra caja":
                selectedValues.other_affiliation?.label,
              "Tipo de pensionado": "Pensionado aportante",
            },
          },
          {
            "Datos del afiliado": {
        ...
        const bodyRequest: CreateRequest = {
          ...
          campaignId: CAMPAIGN_ID_AFFILIATIONS_APORTANT,
          statusId: 1,
          ...
          form: {
            ...
            content: {
              sapData: {
                ...
                NIFEntidad: data.entity.Nit.slice(0, -1),
                DigitoVerifica: data.entity.Nit.slice(-1),
                ...
                TipoActividad: "03",
                CategoriaAportante: selectedValues.modality_affiliation?.value,
                SPCACTEcon: "",
                TipoAfiliado: "2",
        ...
        router.push(
          "/menu-affiliations/pensioner/pensioner-aportant/filing-code"
        );
      }
    } catch (error) {
      console.log(error);
  ...
  const InitData = () => {
    ...
    setSelectedValues({
      ...
      code_eps: data.income.code_eps,
      other_affiliation: data.income.other_affiliation,
      modality_affiliation: data.modality_input,
    });
    setIsLoading(false);
  };

  useEffect(() => {
    download(PENSIONER_AUTORIZATION);
  }, [reload]);
    ...
    <div className="w-full flex flex-col gap-4">
      ...
      <div className="w-full flex flex-col gap-12">
        ...
        <div className="w-full max-w-[912px] flex flex-col gap-6">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 border-2 border-principal-180 rounded-full flex items-center justify-center">
              <h4 className="text-principal-180 text-sm font-semibold">5</h4>
            </div>
            <h2 className="text-principal-180 text-2xl font-semibold">
              Modalidad de aporte
            </h2>
          ...
          <p className="text-principal-450">
            Valida los beneficios que obtienes según tu modalidad de aporte:
          </p>
          <div className="flex items-center justify-start gap-4 w-full">
            <div className="bg-principal-150 p-8 rounded-2xl shadow-lg w-full">
              <h4 className="text-principal-180">
                <strong>Aportando el 0,6% de tu mesada,</strong> tendrás acceso
                a los siguientes servicios: recreación, turismo social y
                capacitación. Ten en cuenta que este aporte{" "}
                <strong>no incluye</strong> beneficios de subsidio monetario,
                subsidio de vivienda o subsidio en especie.
              </h4>
            </div>
            <div className="bg-principal-150 p-8 rounded-2xl shadow-lg w-full">
              <h4 className="text-principal-180 max-w-[358px]">
                <strong>Aportando el 2,0% de tu mesada,</strong> tendrás acceso
                a servicios de recreación, turismo social, capacitación,
                educación y subsidio de vivienda. Ten en cuenta que este aporte{" "}
                <strong>no incluye</strong> beneficios de subsidio monetario.
              </h4>
            </div>
          </div>
          <CustomSelectFormFixed
            className="w-96"
            setValue={() => {}}
            title="Modalidad de aporte"
            placeholder="Selecciona"
            options={optionsModality}
            onChange={(value) => handleChange(value, "modality_affiliation")}
            value={selectedValues.modality_affiliation}
            errors={
              errors.modality_affiliation && (
                <p className="text-principal-500">
                  {errors.modality_affiliation}
                </p>
              )
        ...
        <div className="w-full max-w-[912px] flex flex-col gap-6">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 border-2 border-principal-180 rounded-full flex items-center justify-center">
              <h4 className="text-principal-180 text-sm font-semibold">6</h4>
            </div>
            <h2 className="text-principal-180 text-2xl font-semibold">
              Documentos soporte
            </h2>
          ...
          <div className="flex flex-col gap-6 w-full">
            ...
            <LoadFile
              title={TITLE_DOCS[27]}
              file={files.document_pension_discount}
              setFile={handleSetFile}
              name="document_pension_discount"
              maxSizeMB={5}
              allowedFileTypes={["pdf", "jpg", "png"]}
              errors={
                errors.document_pension_discount && (
                  <p className="text-principal-500">
                    {errors.document_pension_discount}
                  </p>
                )
            ...
            <div className="flex flex-row w-full justify-center mt-10">
              ...
              >
                <p className="font-outfit text-sm font-normal text-principal-180">
                  Haz click para descargar el documento de autorización de
                  descuento de mesada pensional
                </p>
              </Link>
            </div>
          </div>
