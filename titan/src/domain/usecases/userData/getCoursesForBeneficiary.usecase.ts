...
@injectable()
export default class GetCoursesForBeneficiaryUseCase {
  ...
  async execute(token: string): Promise<CourseScheduleTypeList | undefined> {
    return await this.userRannRepository.getCoursesForBeneficiary(token);
  }
}
